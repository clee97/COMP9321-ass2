...
public class MainAPI extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (action.equals("login")){
			...
			currUser = (UserProfile)request.getSession().getAttribute("loggedInUser");
			if (successful){
			...
			newUser.setDateJoined(sqlTimeStamp);
			...
			boolean accepted = friendRequestService.acceptFriendRequest(request, Long.parseLong(request.getParameter("toUser")), Long.parseLong(request.getParameter("fromUser")));
			if (accepted){
				System.out.println("acceptFriendRequest DONE");
				request.getRequestDispatcher("home.jsp").forward(request, response);
			}else{
				System.out.println("acceptFriendRequest NOT DONE");
				request.getRequestDispatcher("home.jsp").forward(request, response);
			}
			...
			List<String> activities = adminService.userActivityReport(request, selectedUser.getId());
			...
			adminService.banUser(request, userID);			
			isAdminSearchResult = 1;
			String nameParam = request.getParameter("name");
			String genderParam = request.getParameter("gender");
			String dobParam = request.getParameter("dob");			
			adminSearchResultParams[0] = nameParam; //selectedUser.getFirstname();
			adminSearchResultParams[1] = genderParam; //selectedUser.getGender();
			adminSearchResultParams[2] = dobParam; //selectedUser.getDob();	
		}else if (action.equals("unban")
				&& (request.getAttribute("isBanned") == null || !request.getAttribute("isBanned").equals("done"))){ //only process when unban btn is clicked
			String userID = request.getParameter("userID");
			adminService.unbanUser(request, userID);
			isAdminSearchResult = 1;
			String nameParam = request.getParameter("name");
			String genderParam = request.getParameter("gender");
			String dobParam = request.getParameter("dob");
			//System.out.println( "MAIN API || nameParam: "+nameParam+ " |||"+genderParam+"||userID: "+userID);			
			adminSearchResultParams[0] = nameParam; //selectedUser.getFirstname();
			adminSearchResultParams[1] = genderParam; //selectedUser.getGender();
			adminSearchResultParams[2] = dobParam; //selectedUser.getDob();				
			//request.getRequestDispatcher("/API?action="+actionParam+"&name="+nameParam+"&gender="+genderParam+"&dob"+dobParam).forward(request, response);
